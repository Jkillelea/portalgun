...
#define EXPANDER_ADDR         ((uint8_t) (0x20))
#define EXPANDER_CNTRL_INPUT  ((uint8_t) (0x00))
#define EXPANDER_CNTRL_OUTPUT ((uint8_t) (0x01))
#define EXPANDER_CNTRL_INVERT ((uint8_t) (0x02))
#define EXPANDER_CNTRL_CONFIG ((uint8_t) (0x03))

#define EXPANDER_PIN_DISP_nRST     ((uint8_t) (1 << 0))
#define EXPANDER_PIN_LED1          ((uint8_t) (1 << 1))
#define EXPANDER_PIN_LED2          ((uint8_t) (1 << 2))
#define EXPANDER_PIN_LED3          ((uint8_t) (1 << 3))
#define EXPANDER_PIN_SPI1_nSS      ((uint8_t) (1 << 4))
#define EXPANDER_PIN_NCP_LBI_EN    ((uint8_t) (1 << 5))
#define EXPANDER_PIN_NCP_LBO       ((uint8_t) (1 << 6))
#define EXPANDER_PIN_DAC_nSHUTDOWN ((uint8_t) (1 << 7))

const char *wifiStatusString(wl_status_t s)
{
    switch (s) {
        case WL_NO_SHIELD:
            return "WL_NO_SHIELD";
...
void scanI2CBus(void)
{
    ...
    for (int addr = 0; addr < (1 << 7); addr++)
    {
        ...
        {
            Serial.printf("Found device at I2C address %d\n", addr);
        }
    }
}

uint8_t PCAWriteReg(uint8_t reg, uint8_t data)
{
...
uint8_t PCAWriteConfig(uint8_t config)
{
    return PCAWriteReg(EXPANDER_CNTRL_CONFIG, config);
}

uint8_t PCAWriteOutput(uint8_t out)
{
    return PCAWriteReg(EXPANDER_CNTRL_OUTPUT, out);
}
...
{
    bool pm = (hour >= 12);
    uint8_t *spHour[] = { spTWELVE, spONE, spTWO, spTHREE, spFOUR, spFIVE, spSIX, spSEVEN, spEIGHT, spNINE, spTEN, spELEVEN };
    size_t   spHourLen[] = { sizeof(spTWELVE), sizeof(spONE), sizeof(spTWO), sizeof(spTHREE), sizeof(spFOUR), sizeof(spFIVE),
        sizeof(spSIX), sizeof(spSEVEN), sizeof(spEIGHT), sizeof(spNINE), sizeof(spTEN), sizeof(spELEVEN) };
...
void setup()
{
    ...
    // Set output pins
    PCAWriteConfig((uint8_t) ~(EXPANDER_PIN_DAC_nSHUTDOWN | EXPANDER_PIN_LED1 | EXPANDER_PIN_LED2 | EXPANDER_PIN_LED3));
    PCAWriteOutput(EXPANDER_PIN_DAC_nSHUTDOWN);

    WiFi.mode(WIFI_STA);
    WiFi.begin("HumiditySucks24", "freezingpoint!");
